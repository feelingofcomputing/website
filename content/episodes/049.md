...
Yeah, so I guess there are two things that I would say, so certainly having a traditional run button was just a simple, lazy choice. There's no reason why it couldn't be a more, live continuous automatic process. It's just that when you're in normal text editing mode, the workflow is right now, you currently press compile, you press run, like you normally would, but there's no reason why we didn't just implement some automatic person rerun feature, like you would expect in a live programming environment or going to go in forward building the _programming with holes_ work that Cyrus and the [Hazel](https://hazel.org) project are exploring. There's no reason why we wouldn't want that as well. And so that's just an orthogonal feature that we just haven't worked on or polished on at all, that does not really an explicit choice.
